...
static const char* const default_kb_map_lc[] = { "1#", "q", "w", "e", "r", "t", "y", "u", "i", "o", "p", LV_SYMBOL_BACKSPACE, "\n",
                                                 "PinYin","ABC", "a", "s", "d", "f", "g", "h", "j", "k", "l", LV_SYMBOL_NEW_LINE, "\n",
                                                 "_", "-", "z", "x", "c", "v", "b", "n", "m", ".", ",", ":", "\n",
...
static const lv_btnmatrix_ctrl_t default_kb_ctrl_lc_map[] = {
    ...
    LV_KEYBOARD_CTRL_BTN_FLAGS | 3,LV_KEYBOARD_CTRL_BTN_FLAGS | 3 ,LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_BTNMATRIX_CTRL_CHECKED | 7,
...
static char pinyin_str[5][8] = { " ", " ", " ", " ", " " };
static const char* const default_kb_map_lc_pinyin_sel[] = { "<", pinyin_str[0], pinyin_str[1], pinyin_str[2], pinyin_str[3],pinyin_str[4], ">", "" };
static const lv_btnmatrix_ctrl_t default_kb_ctrl_lc_pinyin_sel_map[] = {
   LV_KB_BTN(2), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3),LV_KB_BTN(2)
};
static const char* const default_kb_map_lc_pinyin[] = { "1#", "q", "w", "e", "r", "t", "y", "u", "i", "o", "p", LV_SYMBOL_BACKSPACE, "\n",
                                                 "ABC", "a", "s", "d", "f", "g", "h", "j", "k", "l", LV_SYMBOL_NEW_LINE, "\n",
                                                 "_", "-", "z", "x", "c", "v", "b", "n", "m", ".", ",", ":", "\n",
                                                 LV_SYMBOL_KEYBOARD, LV_SYMBOL_LEFT, " ", LV_SYMBOL_RIGHT, LV_SYMBOL_OK, ""
};

static const lv_btnmatrix_ctrl_t default_kb_ctrl_lc_pinyin_map[] = {
    LV_KEYBOARD_CTRL_BTN_FLAGS | 5, LV_KB_BTN(4), LV_KB_BTN(4), LV_KB_BTN(4), LV_KB_BTN(4), LV_KB_BTN(4), LV_KB_BTN(4), LV_KB_BTN(4), LV_KB_BTN(4), LV_KB_BTN(4), LV_KB_BTN(4), LV_BTNMATRIX_CTRL_CHECKED | 7,
    LV_KEYBOARD_CTRL_BTN_FLAGS | 3 ,LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_BTNMATRIX_CTRL_CHECKED | 7,
    LV_BTNMATRIX_CTRL_CHECKED | LV_KB_BTN(1), LV_BTNMATRIX_CTRL_CHECKED | LV_KB_BTN(1), LV_KB_BTN(1), LV_KB_BTN(1), LV_KB_BTN(1), LV_KB_BTN(1), LV_KB_BTN(1), LV_KB_BTN(1), LV_KB_BTN(1), LV_BTNMATRIX_CTRL_CHECKED | LV_KB_BTN(1), LV_BTNMATRIX_CTRL_CHECKED | LV_KB_BTN(1), LV_BTNMATRIX_CTRL_CHECKED | LV_KB_BTN(1),
    LV_KEYBOARD_CTRL_BTN_FLAGS | 2, LV_BTNMATRIX_CTRL_CHECKED | 2, 6, LV_BTNMATRIX_CTRL_CHECKED | 2, LV_KEYBOARD_CTRL_BTN_FLAGS | 2
};
...
static const char** kb_map[] = {
    ...
    (const char**)default_kb_map_lc_pinyin,
    (const char**)default_kb_map_lc_pinyin_sel,
    (const char**)NULL,
};
static const lv_btnmatrix_ctrl_t* kb_ctrl[] = {
    ...
    default_kb_ctrl_lc_pinyin_map,
    default_kb_ctrl_lc_pinyin_sel_map,
    NULL,
};
...
lv_obj_t* lv_keyboard_create(lv_obj_t* parent)
{
    ...
    keyboard->btn_sel = btn_sel_create(obj);
    return obj;
}
static void btn_sel_click_event_handler(lv_event_t* e)
{
...
//创建候选框
static lv_obj_t* btn_sel_create(lv_obj_t* obj)
{
    lv_keyboard_t* keyboard = (lv_keyboard_t*)obj;

    lv_obj_t* btn_sel = lv_btnmatrix_create(lv_obj_get_parent(obj));
    lv_btnmatrix_set_map(btn_sel, kb_map[LV_KEYBOARD_MODE_PINYIN_SEL]);
    lv_btnmatrix_set_ctrl_map(btn_sel, kb_ctrl[LV_KEYBOARD_MODE_PINYIN_SEL]);
    lv_obj_set_size(btn_sel, LV_PCT(100), LV_SIZE_CONTENT);
    lv_obj_align_to(btn_sel, keyboard, LV_ALIGN_OUT_TOP_MID, 0, 8);
    lv_obj_set_style_border_width(btn_sel, 0, 0);
    lv_obj_add_event_cb(btn_sel, lv_keyboard_def_event_cb, LV_EVENT_VALUE_CHANGED, (void*)2);
    lv_obj_set_style_base_dir(btn_sel, LV_BASE_DIR_LTR, 0);
    lv_obj_add_flag(btn_sel, LV_OBJ_FLAG_HIDDEN);
...
static void pinyin_input_proc(lv_keyboard_t* pinyin)
{
    pinyin->py_str = pinyin_search(&pinyin_str[0][1], &pinyin->num);
    pinyin->py_page = 0;
    ...
    if(pinyin->py_str == NULL) { return; }
    //填充到buf
    for(uint8_t i = 0; i < pinyin->num && i < 4; i++)
    {
        for(uint8_t j = 0; j < 3; j++)
        {
            pinyin_str[i + 1][j] = pinyin->py_str[i * 3 + j];
        }
    }
}

static void pinyin_page_proc(lv_keyboard_t* pinyin, uint8_t btn) //0: prev  1: next
{
    if(btn == 0)
    {
        if(pinyin->py_page)
    ...
    {
        if(pinyin->py_page < (pinyin->num - 1) / 4)
            pinyin->py_page++;
    }
    ...
    for(uint8_t i = 0; i < pinyin->num && i < 4; i++)
    {
        for(uint8_t j = 0; j < 3; j++)
        {
            pinyin_str[i + 1][j] = pinyin->py_str[pinyin->py_page * 12 + i * 3 + j];
        }
    }
}

static void pinyin_clear(lv_keyboard_t* pinyin)
{
    pinyin->py_str = NULL;
    pinyin->py_page = 0;
    pinyin->num = 0;
...
void lv_keyboard_def_event_cb(lv_event_t* e)
{
    ...
    static lv_keyboard_t* keyboard;

    if(index == 1)
    {
        keyboard = (lv_keyboard_t*)obj;
    ...
    if(strcmp(txt, "abc") == 0) {
        ...
        lv_obj_add_flag(keyboard->btn_sel, LV_OBJ_FLAG_HIDDEN);
        return;
    }
    ...
    else if(strcmp(txt, "PinYin") == 0) {
        keyboard->mode = LV_KEYBOARD_MODE_PINYIN;
        lv_btnmatrix_set_map(obj, kb_map[LV_KEYBOARD_MODE_PINYIN]);
        lv_btnmatrix_set_ctrl_map(obj, kb_ctrl[LV_KEYBOARD_MODE_PINYIN]);
        lv_obj_clear_flag(keyboard->btn_sel, LV_OBJ_FLAG_HIDDEN);
        return;
    }
    ...
    if(strcmp(txt, "Enter") == 0 || strcmp(txt, LV_SYMBOL_NEW_LINE) == 0) {
        ...
        {
            uint16_t len = strlen((char*)pinyin_str[0]);
            if(len == 1)
            {
                if(pinyin_str[0][0] == ' ') //删除编辑框的内容
                    lv_textarea_del_char(keyboard->ta);
                else
                    pinyin_str[0][0] = ' ';
            }
            ...
            {
                pinyin_str[0][len - 1] = 0;
            }
            else
            {
                lv_textarea_del_char(keyboard->ta);
            }
            pinyin_input_proc(keyboard);
        }
    }
        ...
        else
        {
            ...
            if(btn_id == 0)
            {
                pinyin_page_proc(keyboard, 0);
            }
            else if(btn_id == 6 && index == 2)
            {
                pinyin_page_proc(keyboard, 1);
            }
            else if(btn_id >= 1 && btn_id <= 5 && index == 2)
            {
                if(btn_id >= 2)
                    if(strcmp(txt, " ") != 0)
                    {
            ...
            else
            {
                if(txt[0] >= 'a' && txt[0] <= 'z')
                {
                    if(strlen((char*)pinyin_str[0]) < 7)
                    {
                        strcat(pinyin_str[0], txt);
                    }
                    pinyin_input_proc(keyboard);
                }
                else
...
static void lv_keyboard_constructor(const lv_obj_class_t* class_p, lv_obj_t* obj)
{
    ...
    lv_obj_add_event_cb(obj, lv_keyboard_def_event_cb, LV_EVENT_VALUE_CHANGED, (void*)1);
